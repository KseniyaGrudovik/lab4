package bsu.rfe.java.group7.lab4.Grudoik.varB3;
...
public class GraphicsDisplay extends JPanel{
    ...
    private double minX;
    private double maxX;
    private double trueMaxX;
    private double trueMinX;
    private double minY;
    private double maxY;
...
// Перо для рисования графика
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_ROUND, 10.0f, new float[]{6,3,6,3,6,3,1,3,1,3,1,3}, 0.0f);
...
// Шаг 2 - Организовать цикл по всем точкам графика
        ...
        Ellipse2D.Double maxYmarker = new Ellipse2D.Double();
        Point2D.Double maxYcenter = xyToPoint(0, maxY);
        Point2D.Double maxYcorner = shiftPoint(maxYcenter, 5.5, 5.5);
        maxYmarker.setFrameFromCenter(maxYcenter, maxYcorner);
        canvas.draw(maxYmarker); // Начертить контур маркера
        canvas.fill(maxYmarker);

        Ellipse2D.Double minYmarker = new Ellipse2D.Double();
        Point2D.Double minYcenter = xyToPoint(0, minY);
        Point2D.Double minYcorner = shiftPoint(minYcenter, 5.5, 5.5);
        minYmarker.setFrameFromCenter(minYcenter, minYcorner);
        canvas.draw(minYmarker); // Начертить контур маркера
        canvas.fill(minYmarker);

        Ellipse2D.Double maxXmarker = new Ellipse2D.Double();
        Point2D.Double maxXcenter = xyToPoint(trueMaxX, 0);
        Point2D.Double maxXcorner = shiftPoint(maxXcenter, 5.5, 5.5);
        maxXmarker.setFrameFromCenter(maxXcenter, maxXcorner);
        canvas.draw(maxXmarker); // Начертить контур маркера
        canvas.fill(maxXmarker);

        Ellipse2D.Double minXmarker = new Ellipse2D.Double();
        Point2D.Double minXcenter = xyToPoint(trueMinX, 0);
        Point2D.Double minXcorner = shiftPoint(minXcenter, 5.5, 5.5);
        minXmarker.setFrameFromCenter(minXcenter, minXcorner);
        canvas.draw(minXmarker); // Начертить контур маркера
        canvas.fill(minXmarker);
...
// Вывести надпись в точке с вычисленными координатами
            ...
            String MaxY = Double.toString(maxY);
            String MinY = Double.toString(minY);

            bounds = axisFont.getStringBounds(MaxY, context);
            canvas.drawString(MaxY, (float)(labelPos.getX() - 75),
                    (float)(labelPos.getY() - bounds.getY()/2));

            bounds = axisFont.getStringBounds(MinY, context);
            labelPos = xyToPoint(0, minY);
            canvas.drawString(MinY, (float)(labelPos.getX() - 75 ),
                    (float)(labelPos.getY() - bounds.getY()/2 - 10));
...
// Вывести надпись в точке с вычисленными координатами
            ...
            String MaxX = Double.toString(trueMaxX);
            String MinX = Double.toString(trueMinX);
            //System.out.println(MaxX);
            //System.out.println(MinX);

            bounds = axisFont.getStringBounds(MinX, context);
            labelPos = xyToPoint(trueMinX, 0);
            canvas.drawString(MinX, (float)(labelPos.getX() - bounds.getWidth()/2 - 7),
                    (float)(labelPos.getY() + bounds.getY() + 10));
            bounds = axisFont.getStringBounds(MaxX, context);
            labelPos = xyToPoint(trueMaxX, 0);
            canvas.drawString(MaxX, (float)(labelPos.getX() - bounds.getWidth()/2),
                    (float)(labelPos.getY() + bounds.getY() + 10));
        }
